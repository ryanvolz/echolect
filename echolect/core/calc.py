...
def calc_power_factor(noise_est, cal_est, cal_temp, bandwidth):
    return sp.constants.Boltzmann*bandwidth*cal_temp/(cal_est - noise_est)
...
def altitude_from_range(range_m, azimuth_deg, elevation_deg,
    ...
    range_m = np.array(range_m, copy=False, ndmin=1).ravel()
    ...
    # known values
    az = math.radians(azimuth_deg) # beam azimuth angle
    el = math.radians(elevation_deg) # beam elevation angle
    lat = math.radians(latitude_deg) # site latitude
    h = base_altitude_m # site altitude

    # site position vector in IJK coordinates
    N = a / math.sqrt(1 - esq*math.sin(lat)**2)
    rho_vec = np.matrix((   ((N + h)*math.cos(lat), ),
                            (0, ),
                            ((N*(1 - esq) + h)*math.sin(lat), ) ))

    # rotation matrix from SEZ to IJK
    rot_ang = math.radians(-90) + lat
    rot = np.matrix((   (math.cos(rot_ang), 0, -math.sin(rot_ang)),
                        (0, 1, 0),
                        (math.sin(rot_ang), 0, math.cos(rot_ang)) ))

    alt_m = np.empty(range_m.shape[0])
    for k, r in enumerate(range_m):
        # position vector from site in SEZ coordinates
        r_vec = np.matrix((  (-r*math.cos(el)*math.cos(az), ),
        ...
        C = (esq**2)*F*(R**2) / math.pow(G, 3)
        S = math.pow(1 + C + math.sqrt(C**2 + 2*C), 1./3.)
        P = F / ( 3*((S + 1/S + 1)**2)*(G**2) )
        Q = math.sqrt(1 + 2*(esq**2)*P)
        R0 = (-(P*esq*R) / (1 + Q) + math.sqrt(0.5*(a**2)*(1 + 1/Q)
                - (P*(1 - esq)*Z**2) / (Q*(1 + Q)) - 0.5*P*(R**2)))
        U = math.sqrt((R - esq*R0)**2 + Z**2)
        V = math.sqrt((R - esq*R0)**2 + (1 - esq)*Z**2)
        alt = U*(1 - (b**2 / (a*V)))

        # alt = math.sqrt(r**2 + a**2 - 2*r*a*math.cos(el + math.radians(90))) - a + h # simpler and almost as accurate (within meters)
